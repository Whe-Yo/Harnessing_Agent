...
1. **룰 파일 주입 (최우선)**: `rules/AGENTS.md`를 에이전트가 **자동 로드하는** 룰 파일에 건다. 틀린 파일명에 넣으면 룰이 조용히 미적용된다.
   - **기본은 `AGENTS.md` 표준**: 대부분의 에이전트(Codex, Antigravity, 2026+ Claude Code 등)가 이 이름을 읽는다. 전 에이전트 파일명 카탈로그는 만들지 않는다(stale 부채).
   - **알려진 예외만 분기**: Gemini CLI → `GEMINI.md`, Cursor → `.cursor/rules`(또는 `.cursorrules`). 미상이면 해당 에이전트 문서를 확인하거나 사용자에게 룰 파일 위치를 묻는다.
   - **복붙보다 import 우선**: 에이전트가 파일 참조(`@import`/경로 지정)를 지원하면 하네스 `rules/AGENTS.md`를 한 줄로 import한다 — 기존 사용자 규칙 보존 + 하네스 업데이트 자동 전파. 미지원이면 내용을 **병합**(덮어쓰기 금지).
   - **스코프**: 하네스 규칙은 모든 프로젝트에 적용되므로 에이전트의 **글로벌** 룰 파일에 건다.
   - **검증은 결정론적으로**: 주입 후 그 파일이 올바른 경로에 존재하고 규칙/​import를 포함하는지 직접 확인한다. "에이전트가 자기 컨텍스트에서 보이는지"라는 자가 보고에 의존하지 않는다. 확실히 하려면 새 세션을 띄워 에이전트가 규칙을 따르는지(예: boost 우선 실행) 사용자가 관찰.
